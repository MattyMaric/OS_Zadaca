...
#include <sys/shm.h>
#include <unistd.h>
int *ULAZ; // Oznacava da li je thread u procesu biranja broja
int *BROJ; // Niz koji sadrzi brojeve pridodane na svaki thread 
int *REZ; // Oznacava rezervacije, -1 oznacava da je stol slobodan, ostalo da nije
int brSt, brDr;
int max(){
	int najv = BROJ[0];
	for(int i = 0; i<brSt; i++){		//Vraca najvecu vrijednost iz BROJ niza
		if(BROJ[i] > najv){
			najv = BROJ[i];
		}
	}
...
int sve_zauzeto(){
	for(int i = 0; i<brSt; i++){
		if(REZ[i] == -1){			//Vraca 1 ako su svi stolovi zauzeti, ako nisu vraca 0
			return 0;
		}
	}
	return 1;
...
void udji_u_kriticni_odsjecak(int i){
	ULAZ[i] = 1;
	BROJ[i] = max() + 1;			// Koristi bakery algoritam da pristupi kriticnom odsjecku
	ULAZ[i] = 0;					// Thread "i" dobije broj i ceka svoj red da pristupi odsjecku
...
void *provjeri_stol(void *pdr){
	...
	char stanje[brSt ];
        for(int i = 0; i<brSt; i++){
                stanje[i] = (REZ[i] == -1) ? '-' : REZ[i] + 1 + '0'; 	//Ako je REZ[i] == -1 onda ce postaviti -, ako nije, onda potavlja vrijednost REZ[i] +1 +'0'
        }
        stanje[brSt] = '\0';
	if(REZ[rand_tbl] == -1){
		REZ[rand_tbl] = dr;
